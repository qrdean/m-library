...
@suite class HerosTest {
    ...
    @test public get() {
        return chai.request(HerosTest.server).get(`${HerosTest.BASE_URI}/${HerosTest.hero}`).then(response => {
            response.should.have.status(200);
    ...
    @test public put() {
        ...
        return chai.request(HerosTest.server).put(`${HerosTest.BASE_URI}/${HerosTest.hero}`)
            .send(data)
            .then(response => {
                response.should.have.status(200);
                response.body.should.be.a("object");
